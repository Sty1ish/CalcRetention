...
namespace BI.API_Server.Controllers
{
    ...
    /// </summary>
    public class RollingRetentionDataController : ControllerBase
    {
        private ILogger<RollingRetentionDataController> _logger;

        private readonly dataLoaderRollingRetention _retentionProvider;

        public RollingRetentionDataController(IServiceProvider serviceProvider)
        {
            var loggerFactory = serviceProvider.GetService<ILoggerFactory>();
            _logger = loggerFactory.CreateLogger<RollingRetentionDataController>();
            ...
            _retentionProvider = new dataLoaderRollingRetention(serviceProvider, analyticsDB);
        }

        // GET: api/RetentionDatas.
        [HttpGet]
        [Route("retention/rolling")]
        public string GetRetentionData(string dateTime, string country, string market, int analyticsDate)
        {
            _logger.LogInformation($"[Rolling Retention Controller Filter] Date: {dateTime}, analyticsDate : {analyticsDate}, Country: {country}, Market: {market}.");
